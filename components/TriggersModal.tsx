...
export const TriggersModal: React.FC<TriggersModalProps> = ({ isOpen, onClose, onSelectPrompt }) => {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
      <div className="w-full max-w-2xl bg-[#1e1f20] rounded-3xl border border-white/10 shadow-2xl overflow-hidden flex flex-col max-h-[85vh] animate-message">
        
        {/* Header */}
        <div className="p-6 border-b border-white/5 bg-[#282a2c] flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-white tracking-tight">Biblioteca de Comandos</h2>
        ...
        {/* Search */}
        <div className="px-6 py-4 bg-[#131314] border-b border-white/5">
          <div className="relative">
            ...
            <input 
              ...
              className="w-full bg-[#1e1f20] text-sm text-gray-200 pl-12 pr-4 py-3 rounded-2xl border border-transparent focus:border-blue-500 outline-none transition-all"
            />
          </div>
        </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {category.items.map((item, itemIdx) => (
                  <button
                    ...
                    className="group flex flex-col items-start p-4 rounded-2xl bg-white/5 border border-white/5 hover:border-blue-500/50 hover:bg-blue-500/5 transition-all text-left relative overflow-hidden"
                  >
                    <div className="flex items-center justify-between w-full mb-1">
                      <span className="text-sm font-bold text-gray-200 group-hover:text-blue-400 transition-colors">{item.title}</span>
        ...
        <div className="p-4 bg-[#282a2c] border-t border-white/5 flex justify-center">
           <p className="text-[10px] text-gray-600 uppercase font-black tracking-widest">Selecione um prompt para carregar no chat</p>
        </div>
      </div>
    </div>
  );
